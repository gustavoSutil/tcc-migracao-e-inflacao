...
def diagnostico_csv(path, nrows=5, encoding='utf-8', sep=','):
    """
    Mostra os tipos detectados pelo pandas e as primeiras linhas do CSV bruto.
    """
...
def load_censo_2022(path):
    ...
    return df[['municipio', 'ano', 'pop_censo_2022']]
...
def load_tse(path):
    ...
    df['municipio'] = df['id_municipio'].astype(str).str.zfill(7)
    df['ano'] = df['ano'].astype(int)
    if 'total_eleitores' in df.columns:
        df['eleitores'] = pd.to_numeric(df['total_eleitores'], errors='coerce')
    return df[['municipio', 'ano', 'eleitores']]